...
const inp =
  ...
  'outline-none focus:bg-white focus:border-[#E91E63]/30 focus:ring-4 focus:ring-[#E91E63]/8 transition-all';
...
function OrderCard({ image, boutique, status, statusIcon, statusColor, price, action }) {
  return (
    <div className="flex items-center gap-5 rounded-2xl p-4 border border-gray-100 bg-white hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
      ...
      <button className="shrink-0 text-xs font-semibold px-5 py-2.5 rounded-xl border border-gray-200 text-gray-500 hover:bg-[#E91E63] hover:text-white hover:border-[#E91E63] transition-all duration-200">
        {action}
      </button>
    </div>
  );
...
export default function AccountDetails() {
  ...
  const orders = [
    { id:1, image:new1Img, boutique:'The Silk Road', status:'In Transit', statusColor:'#16a34a', statusIcon:<ArrowIcon/>, price:'$420',   action:'Track Order'  },
    { id:2, image:new4Img, boutique:'Maison Noir',   status:'Delivered',  statusColor:'#15803d', statusIcon:<CheckIcon/>,  price:'$1,250', action:'View Receipt' },
  ];
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-[#fafafa]">
      ...
      <header className="shrink-0 bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between z-50 shadow-[0_1px_12px_rgba(0,0,0,0.04)]">
        <span
          ...
          className="text-2xl font-black text-[#E91E63] tracking-tight uppercase cursor-pointer select-none hover:opacity-80 transition-opacity"
        >
          Maison
        </span>

        <div className="flex items-center gap-5 text-gray-400">
          <button className="hover:text-[#E91E63] transition-colors"><HeartIcon/></button>
          <button className="hover:text-[#E91E63] transition-colors"><BagIcon/></button>
          <button className="hover:text-[#E91E63] transition-colors"><ChatIcon/></button>
          <button
            className="text-[#E91E63]"
            onClick={() => navigate('/account')}
          >
            <UserIcon s={22} c="#E91E63"/>
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside
          ...
          style={{
            background: 'linear-gradient(160deg, #c2185b 0%, #e91e63 45%, #f06292 100%)',
          }}
        >
          {/* Brand mark */}
        ...
        <main className="flex-1 overflow-y-auto px-10 py-8 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-3xl shadow-[0_4px_24px_rgba(0,0,0,0.05)] p-6 flex items-center gap-6">
            ...
            <div className="relative shrink-0">
              <div className="w-20 h-20 rounded-2xl flex items-center justify-center"
                ...
                <UserIcon s={38} c="#E91E63"/>
              </div>
              <button className="absolute -bottom-1 -right-1 w-6 h-6 bg-[#E91E63] rounded-lg flex items-center justify-center shadow-lg hover:scale-110 transition-transform">
                <PencilIcon/>
              </button>
            </div>
            ...
            <button className="shrink-0 border-2 border-[#E91E63] text-[#E91E63] px-6 py-2.5 rounded-2xl text-sm font-bold hover:bg-[#E91E63] hover:text-white transition-all duration-200">
              Edit Profile
            </button>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-[0_4px_24px_rgba(0,0,0,0.05)] p-8">
            ...
            <div className="mt-8">
              <button
                ...
                style={{ background: 'linear-gradient(135deg,#e91e63,#c2185b)', boxShadow:'0 8px 24px rgba(233,30,99,0.30)' }}
              >
                Save Changes
              </button>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-[0_4px_24px_rgba(0,0,0,0.05)] p-8">
            <div className="flex items-center justify-between mb-2">
              ...
              <button className="text-[#E91E63] text-sm font-bold hover:underline">View All Orders</button>
            </div>
          ...
          <footer className="mt-2 py-8 border-t border-gray-100">
            <div className="grid grid-cols-4 gap-8">
              <div>
                <div className="text-lg font-black text-[#E91E63] uppercase mb-3">Maison</div>
              ...
              {[
                ...
                <div key={col.title}>
                  <h6 className="text-[9px] font-black text-[#E91E63] uppercase tracking-widest mb-4">{col.title}</h6>
                  <ul className="space-y-2.5">
                    {col.links.map(l => (
                      <li key={l} className="text-xs text-gray-400 hover:text-[#E91E63] cursor-pointer transition-colors">{l}</li>
                    ))}
                  </ul>
                </div>
              ))}
